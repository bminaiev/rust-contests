...
fn solve_task(t: &Task, load_snakes: Vec<Vec<Pos>>) -> State {
    ...
    let mut next_states: Vec<_> = (0..20)
        .into_par_iter()
        .map(|par_id| {
            let mut state = start_state.clone();
            let mut rnd = Random::new(3245234 + par_id);
            ...
            let mut sa = SimulatedAnnealing::new(
                ...
                rnd.gen_double() * 10.0 + 0.2,
                0.1,
                0,
            );
            sa.with_out_prefix(format!("[start_score = {}, {par_id}] ", state.score));
            while sa.should_continue() {
                let snake_id = rnd.gen(0..state.snakes.len());
                let positions = &state.snakes[snake_id].positions;
                let len = positions.len();
                let stay_idx = rnd.gen(0..len);
                let mut last = positions[stay_idx];
                ...
                if state.can_apply(&new_change) && sa.should_go(state.score + score_delta) {
                    state.apply(&new_change)
                }
            }
            state
        })
    ...
    next_states.sort_by_key(|s| -s.score);

    let state = next_states[0].clone();
    let mut cnt_used = 0;
