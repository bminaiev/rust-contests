...
impl Solver {
    pub fn new(start: &[usize]) -> Self {
        ...
        let mut rnd = Random::new(78977118);
    ...
    pub fn set(&mut self, pos: usize, value: usize) {
        self.sum_st.update(pos..pos + 1, self.mapping[value]);
        self.xor_st.update(pos..pos + 1, value);
    }
...
fn solve(input: &mut Input) {
    ...
    impl ParallelJob for Job {
        fn read_input(&mut self, input: &mut Input) {
            ...
            self.queries = gen_vec(cnt_q, |_| {
                let q_type = input.usize();
                if q_type == 1 {
                    Query::Change(input.usize() - 1, input.usize())
                } else {
                    assert_eq!(q_type, 2);
        ...
        fn solve(&mut self) {
            let mut res = 0;
            let mut solver = Solver::new(&self.s);
