...
pub struct PlacementGroupVms {
    ...
    cnt_by_machine: HashMap<MachineId, usize>,
}
    ...
    if let Some(any_vm) = info.any_vm_id() {
        for &vm_id in info.id_to_part.keys() {
            if pg.network_affinity_type == 1
                && created[vm_id].machine.dc != created[any_vm].machine.dc
            {
                return true;
            }
            if pg.rack_affinity_type == 1
                && created[vm_id].machine.get_rack() != created[any_vm].machine.get_rack()
            {
                return true;
            }
        }
