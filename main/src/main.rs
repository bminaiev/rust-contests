...
use crate::algo_lib::geometry::nearest_points::find_nearest_points;
use crate::algo_lib::geometry::point::{PointT, PointWithIdT};
use crate::algo_lib::geometry::range_tree::RangeTree;
    ...
    for i in 0..n_points.len() {
        ...
        range_tree
            .iter_rect(
                xs,
                ys,
                |points: &[PointWithIdT<i64>]| {
                    for p2 in points.iter() {
                        if p2.id() == i {
                            continue;
                        }
                        let result = p2.p.dist2(&p.point)
                            * n_points[p2.id()].mass as i64;
...
fn add_sweep_candidate(stack: &mut Vec<SweepCandidate>, point: MovingPoint) {
    ...
    let until = stack
        ...
        .map_or(
            INF,
            |last| { (x * slope - last.x * last.slope) / (slope - last.slope) },
        );
    stack
        .push(SweepCandidate {
            x,
...
fn update_moving_half(
    ...
    for event in events {
        ...
        if event.kind == 0 {
            ...
            let cur_res = dist * dist * (candidate.mass as f64)
                * (event.point.mass as f64) / denom;
...
fn main() {
    ...
    let output = crate::algo_lib::io::output::Output::new_stdout();
    run(input, output);
}
pub mod algo_lib {
pub mod collections {
...
impl Line {
    ...
    pub fn norm(&mut self) {
        if self.a > OrdF64::EPS {} else if self.a < -OrdF64::EPS {
            self.a = -self.a;
            self.b = -self.b;
            self.c = -self.c;
        } else if self.b > OrdF64::EPS {} else if self.b < -OrdF64::EPS {
            self.b = -self.b;
            self.c = -self.c;
        }
    }
    ...
    pub fn intersect(&self, other: &Self) -> Option<Point> {
        ...
        debug_assert!(
            self.abs_dist(& res).eq_with_eps(& OrdF64::ZERO, OrdF64::SMALL_EPS),
            "line = {:?}, p = {:?}, dist = {:?}", self, res, self.abs_dist(& res)
        );
        debug_assert!(
            other.abs_dist(& res).eq_with_eps(& OrdF64::ZERO, OrdF64::SMALL_EPS),
            "line = {:?}, p = {:?}, dist = {:?}", other, res, self.abs_dist(& res)
        );
        Some(res)
    }
...
pub mod nearest_points {
use crate::algo_lib::{
    geometry::{line::Line, point::{PointT, PointWithIdT}},
...
use std::ops::Range;
use crate::algo_lib::{
    geometry::point::PointWithIdT, misc::binary_search::binary_search_first_true,
};
...
impl RangeTree {
    ...
    fn iter_rect_impl<F: FnMut(&[Point])>(
        ...
        if x.start <= self.xs.start && self.xs.end <= x.end {
            let from = binary_search_first_true(
                0..self.points.len(),
                |idx| { self.points[idx].p.y >= y.start },
            );
            let to = binary_search_first_true(
                0..self.points.len(),
                |idx| self.points[idx].p.y >= y.end,
            );
